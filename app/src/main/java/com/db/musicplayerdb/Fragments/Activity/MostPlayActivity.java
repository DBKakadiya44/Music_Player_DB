...
import com.db.musicplayerdb.databinding.ActivityMostPlayBinding;
...
public class MostPlayActivity extends AppCompatActivity {
    public static ActivityMostPlayBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityMostPlayBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        Window window = this.getWindow();
        window.clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
        window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        window.setStatusBarColor(ContextCompat.getColor(this, R.color.c4));

        isCreated = true;
        context = MostPlayActivity.this;
        ...
        musicdata = MostPlayMusic;

        Collections.sort(musicdata, new PlayCountComparator());

        UpdateBottom.updateAllBottom();

        if(MostPlayMusic.isEmpty()){
            binding.textView5.setText("0 Songs");
            binding.noText.setVisibility(View.VISIBLE);
        }else {
            binding.textView5.setText(MostPlayMusic.size()+" Songs");
            binding.noText.setVisibility(View.INVISIBLE);

            MostPlayAdapter adapter = new MostPlayAdapter(MostPlayActivity.this , musicdata);
            RecyclerView.LayoutManager manager = new LinearLayoutManager(MostPlayActivity.this);
            binding.mostrecycler.setLayoutManager(manager);
            binding.mostrecycler.setAdapter(adapter);
    ...
    public class PlayCountComparator implements Comparator<MusicModel> {
        @Override
        public int compare(MusicModel audio1, MusicModel audio2) {
            return Integer.compare(audio2.getPlayCount(), audio1.getPlayCount());
        }
        ...
        if (CurrentMusic.isEmpty()) {
            ...
            binding.bplay.setOnClickListener(v -> {
                ...
                if(MostPlayMusic.isEmpty()){
                    ...
                    MostPlayAdapter adapter = new MostPlayAdapter(context , musicdata);
