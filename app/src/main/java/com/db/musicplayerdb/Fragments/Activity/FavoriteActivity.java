...
import com.db.musicplayerdb.databinding.ActivityFavoriteBinding;
...
public class FavoriteActivity extends AppCompatActivity {
    public static ActivityFavoriteBinding binding;
    public static ArrayList<MusicModel> Favmusic = new ArrayList();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Favmusic = FavCode.loadArrayListFromSharedPreferences(FavoriteActivity.this);
        Collections.reverse(Favmusic);
        ...
        if (Favmusic.size() == 0) {
            ...
            binding.textView5.setText(Favmusic.size() + " Songs");

            FavoriteAdapter adapter = new FavoriteAdapter(FavoriteActivity.this, Favmusic);
            RecyclerView.LayoutManager manager = new LinearLayoutManager(FavoriteActivity.this);
            binding.favrecycler.setLayoutManager(manager);
    ...
    private void startAllMusic() {
        ...
        if (mainmediaPlayer.isLooping()) {
            try {
                mainmediaPlayer.reset();
                mainmediaPlayer.setDataSource(Favmusic.get(currentIndex).getPath());
                mainmediaPlayer.prepare();
                mainmediaPlayer.start();

                SplashActivity.editor.putString("LastMusicPlay", Favmusic.get(currentIndex).getName());
        ...
        if (CurrentMusic.isEmpty()) {
            ...
            binding.bplay.setOnClickListener(v -> {
                ...
                if (Favmusic.size() == 0) {
                    ...
                    FavoriteAdapter adapter = new FavoriteAdapter(context, Favmusic);
