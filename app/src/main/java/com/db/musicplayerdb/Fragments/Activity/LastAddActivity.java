...
import com.db.musicplayerdb.Adapter.FavoriteAdapter;
...
import com.db.musicplayerdb.databinding.ActivityLastAddBinding;
...
public class LastAddActivity extends AppCompatActivity {
    public static ActivityLastAddBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityLastAddBinding.inflate(getLayoutInflater());
        ...
        context = LastAddActivity.this;
        isCreated = true;

        UpdateBottom.updateAllBottom();
        getRecentlyAddedAudioLastTwoDays(context);
        ...
        if(musicdata.isEmpty()){
            ...
            LastAddAdapter adapter = new LastAddAdapter(LastAddActivity.this , musicdata);
            RecyclerView.LayoutManager manager = new LinearLayoutManager(LastAddActivity.this);
            binding.lastrecycler.setLayoutManager(manager);
            binding.lastrecycler.setAdapter(adapter);
        ...
        if (CurrentMusic.isEmpty()) {
            ...
            binding.bplay.setOnClickListener(v -> {
                ...
                if(musicdata.isEmpty()){
                    ...
                    LastAddAdapter adapter = new LastAddAdapter(context , musicdata);
    ...
    public static void getRecentlyAddedAudioLastTwoDays(Context context) {
        ...
        long twoDaysAgoInSeconds = System.currentTimeMillis() / 1000 - 2 * 24 * 60 * 60;

        String selection = MediaStore.Audio.Media.DATE_ADDED + " > ?";
        String[] selectionArgs = {String.valueOf(twoDaysAgoInSeconds)};

        try (Cursor cursor = context.getContentResolver().query(uri, projection, selection, selectionArgs, null)) {
            if (cursor != null) {
                int idColumn = cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.TITLE);
                int dataColumn = cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DATA);
                int titleColumn = cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DURATION);
                int artistColumn = cursor.getColumnIndexOrThrow(MediaStore.Audio.Media.ARTIST);

                while (cursor.moveToNext()) {
                    String name = cursor.getString(idColumn);
                    String path = cursor.getString(dataColumn);
                    String time = cursor.getString(titleColumn);
                    String artist = cursor.getString(artistColumn);

                    MusicModel audioFile = new MusicModel(name, path, time, artist,1);
                    musicdata.add(audioFile);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
